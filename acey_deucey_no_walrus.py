...
def acey_deucey():
    ...
    cards = ["0", "Ace"] + [str(_) for _ in range(2, 11)] + ["Jack", "Queen", "King"]
    ...
    class Cards:
        ...
        def higher_than(self, card):
            pos = cards.index(self.get_face())
            return card.get_face() in cards[:pos]
...
""")
    ...
    while money > 0:
        ...
        #   intialises 2 empty cards
        c1, c2 = None, None

        #   loops until the cards have different face values
        while 1:
            c1, c2 = Cards(), Cards()
            if c1.get_face() != c2.get_face(): break
        ...
        bet = int(input(f"Amount in hand: {money}{chr(10)}Enter amount to bet: "))
